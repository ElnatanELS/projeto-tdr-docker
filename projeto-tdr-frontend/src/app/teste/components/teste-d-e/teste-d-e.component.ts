...
@Component({
  selector: 'app-teste-d-e',
  templateUrl: './teste-d-e.component.html',
  styleUrls: ['./teste-d-e.component.css']
})
export class TesteDEComponent implements OnInit {
  value = 0;
  tempo_inicial: any;
  tempo_final: any;
  ...
  ngOnInit() {
    ...
    this.inicializarteste('aleatorio');
  }
    ...
    if (event.keyCode === KEY_CODE.TECLA_1) {
      if (this.lado_esquerdo) {
        ...
        this.diferenca(),
          ...
          (this.verde = false),
          (this.vermelho = false);
          (this.lado_direito = false);
          (this.lado_esquerdo = false);
  ...
  startCountDown(seconds, tipo) {
    ...
    var interval = setInterval(() => {
      ...
      if (counter < 0) {
        ...
        this.tipo_de_cor = this.getRandomInt(2);
        console.log(this.tipo_de_cor, ' cor');

        if (tipo == 1) {
          this.lado_esquerdo = true;
        } else {
          this.lado_direito = true;
  ...
  inicializarteste(tipo_de_teste) {
    ...
    this.tipo_aleatorio = 'Teste Lados';
